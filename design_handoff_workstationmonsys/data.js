/* ============================================================
   WorkstationMonSys — mock data + live simulation
   Exposes: window.WMS = { workstations, alerts, events, helpers... }
   ============================================================ */
(function () {
  ...
  // hostname, department, user, preferred os index
  const SEEDS = [
  ...
  // status plan: 18 healthy, 4 warning, 2 critical, 1 offline
  const STATUS_PLAN = [];
  for (let i = 0; i < 18; i++) STATUS_PLAN.push("healthy");
  for (let i = 0; i < 4; i++) STATUS_PLAN.push("warning");
  for (let i = 0; i < 2; i++) STATUS_PLAN.push("critical");
  STATUS_PLAN.push("offline");
  // assign deterministically but spread out
  ...
  function buildWorkstation(seed, idx) {
    ...
    // target levels by status
    let cpu, ram, diskPct, gpu, gpuTemp, cpuTemp;
    if (status === "critical") {
    ...
    if (status === "offline") { cpu = 0; ram = 0; diskPct = rnd(40, 75); gpu = 0; gpuTemp = 0; cpuTemp = 0; }
    ...
    // keep score coherent with assigned status
    ...
    const mac = Array.from({ length: 6 }, () => rndInt(16, 255).toString(16).padStart(2, "0").toUpperCase()).join(":");

    const stateMins = status === "healthy" ? 0 : rndInt(6, 320);

    return {
      id: hostname.toLowerCase(),
      hostname, dept, user, os,
      status,
      ip: ipBase, mac,
      ...
      disk: { size: disk.s, type: disk.t, usedPct: diskPct, readMBs: status === "offline" ? 0 : rnd(2, 220), writeMBs: status === "offline" ? 0 : rnd(1, 180) },
      gpu: { model: pick(GPU_MODELS), load: gpu, temp: gpuTemp },
      net: {
        ethIn: status === "offline" ? 0 : rnd(0.4, 88), ethOut: status === "offline" ? 0 : rnd(0.2, 42),
  ...
  const workstations = SEEDS.map(buildWorkstation);

  /* ---------- alerts / events ---------- */
  const ALERT_DEFS = [
    { metric: "RAM Usage", unit: "%", action: "Identify memory-heavy applications or schedule a reboot.", sev: "critical", thr: "> 90% for 10m" },
    { metric: "CPU Load", unit: "%", action: "Check for runaway compute jobs; consider workload rebalance.", sev: "critical", thr: "> 95% for 5m" },
    { metric: "Disk Capacity", unit: "%", action: "Free disk space or expand the volume.", sev: "warning", thr: "> 85%" },
    { metric: "CPU Temperature", unit: "°C", action: "Inspect cooling / dust; verify fan curve.", sev: "critical", thr: "> 85°C" },
    { metric: "GPU Temperature", unit: "°C", action: "Reduce GPU workload or improve airflow.", sev: "warning", thr: "> 80°C" },
    { metric: "Agent Heartbeat", unit: "", action: "Verify network link and monitoring agent service.", sev: "critical", thr: "no signal 5m" },
    { metric: "Internet Downlink", unit: "Mbit/s", action: "Check uplink / ISP; test alternate route.", sev: "warning", thr: "< 30% of plan" },
    { metric: "Disk I/O Latency", unit: "ms", action: "Investigate background indexing or failing drive.", sev: "warning", thr: "> 40ms" },
  ];
  ...
  let alertId = 1000;
  function makeAlert(ws, def, ageMin, resolved) {
    return {
      id: ++alertId,
      ws: ws.hostname, wsId: ws.id, dept: ws.dept,
      metric: def.metric, value: def.metric.includes("Temp") ? `${rndInt(80, 96)}${def.unit}` : `${rndInt(86, 99)}${def.unit}`,
  ...
  const alerts = [];
  // critical & warning machines generate active alerts
  workstations.forEach((ws) => {
    if (ws.status === "critical") {
      makeN(ws, 2);
    } else if (ws.status === "warning") {
      if (Math.random() > 0.25) makeN(ws, 1);
    } else if (ws.status === "offline") {
      alerts.push(makeAlert(ws, ALERT_DEFS[5], ws.lastSeenMin, false));
    }
  });
  function makeN(ws, n) {
    const defs = pickDefsFor(ws, n);
    defs.forEach((d) => alerts.push(makeAlert(ws, d, rndInt(2, 180), false)));
  }
  function pickDefsFor(ws, n) {
    const cands = [];
  ...
  for (let i = 0; i < 14; i++) {
    ...
    alerts.push(makeAlert(ws, pick(ALERT_DEFS), rndInt(180, 2880), true));
  }

  // live event feed (most recent first) — built from alerts + benign events
  const events = [];
  ...
  /* ---------- live tick ---------- */
  function tickValue(v, vol, min, max) { return clamp(v + rnd(-vol, vol), min, max); }
  ...
  function fleet() {
    ...
    const avg = (sel) => online.reduce((s, w) => s + sel(w), 0) / Math.max(online.length, 1);
  ...
  window.WMS = {
    workstations, alerts, events,
    OS_LIST, ALERT_DEFS,
    tick, fleet, relTime, uptimeStr,
    byId: (id) => workstations.find((w) => w.id === id),
  };
})();
